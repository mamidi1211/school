...
module.exports = {
	...
	Class.create({name:className,strength:classStrength}).exec(function(error, classes){
		if(error){
			...
			response.view('class/classview',{name : "Recorded", recorded: classes});
		}
	});
...
edit:function(request,response){
	...
	Class.findOne({id:classid}).exec(function(error, classes){
		if(error){
			...
			console.log("##", classes);
			response.view('class/classeditview',{name : "Edited", edited: classes});
		}
	});
},	
...
viewAll:function(request,response){
	...
	Class.find().exec(function(error, classes){
		if(error){
			...
			return response.view('class/classlistview',{name : "Getting All Records", recorded: classes});
		}
...
view:function(request,response){
	...
	Class.findOne({id: classid}).exec(function(error, classes){
        if(error){
        	...
        	return response.view('class/classindividualview',{name : "Getting a record", recorded: classes});
        }
	});
}
